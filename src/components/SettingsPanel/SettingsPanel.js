...
export class SettingsPanel extends Component {
				...
				<Content>
					...
					<Row>
						...
						<Range
							...
							step={0.1}
							value={fontSize}
							onChange={event => changeConfig({ fontSize: event.target.value })}
						/>
					</Row>
					<Row>
	...
	handleResetFontControl = () => {
		const { bgColor, onSetAutoFontColor, changeConfig } = this.props;

		onSetAutoFontColor(true);
		changeConfig({ fontColor: getAutoTextColor(bgColor) });

		this.setState({ isEditingFontColor: false });
	};

	handleChangeFontColor = ({ hex }) => {
		const { onSetAutoFontColor, changeConfig } = this.props;

		onSetAutoFontColor(false);
		changeConfig({ fontColor: hex });
	};
